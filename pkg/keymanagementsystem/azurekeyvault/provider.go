...
import (
	...
	"github.com/deislabs/ratify/pkg/keymanagementsystem/azurekeyvault/types"
...
type akvKMSProviderFactory struct{}

// init calls to register the provider
func init() {
	factory.Register(providerName, &akvKMSProviderFactory{})
}

// Create creates a new instance of the provider after marshalling and validating the configuration
func (f *akvKMSProviderFactory) Create(_ string, certificateStoreConfig config.KeyManagementSystemConfig, _ string) (keymanagementsystem.KeyManagementSystemProvider, error) {
	conf := AKVKMSConfig{}

	certStoreConfigBytes, err := json.Marshal(certificateStoreConfig)
	if err != nil {
		return nil, re.ErrorCodeConfigInvalid.WithError(err).WithComponentType(re.CertProvider)
	}

	if err := json.Unmarshal(certStoreConfigBytes, &conf); err != nil {
		return nil, re.ErrorCodeConfigInvalid.NewError(re.CertProvider, "", re.EmptyLink, err, "failed to parse AKV certificate store configuration", re.HideStackTrace)
	}
...
func (s *akvKMSProvider) GetCertificates(ctx context.Context) ([]*x509.Certificate, keymanagementsystem.KeyManagementSystemStatus, error) {
...
func getCertStatusMap(certsStatus []map[string]string) keymanagementsystem.KeyManagementSystemStatus {
	status := keymanagementsystem.KeyManagementSystemStatus{}
...
func getCertsFromSecretBundle(ctx context.Context, secretBundle kv.SecretBundle, certName string) ([]*x509.Certificate, []map[string]string, error) {
	...
	for block != nil {
		...
		case "CERTIFICATE":
			...
			decodedCerts, err := keymanagementsystem.DecodeCertificates(pemData)
			if err != nil {
...
func (s *akvKMSProvider) validate() error {
	if s.vaultURI == "" {
